                                            ...
                                            <div class="d-flex">
                                                <a href="/dashboard/bank/{{$bank->slug}}/edit"
                                                    class="btn btn-primary shadow btn-xs sharp me-1"><i
                                                        ...
                                                        <form action="/dashboard/bank/{{ $bank->slug }}" method="POST" class="d-inline"
                                                            id="data-{{ $bank->slug }}">
                                                            @method('DELETE')
                                                            @csrf
                                                            <button class="btn btn-danger shadow btn-xs sharp me-1 delete" data-name="{{ $bank->name }}"
                                                              data-slug="{{ $bank->slug }}"><i class='fa fa-trash'></i></button>
                                                          </form>
                                            </div>
                                        </td>
                                        @endforeach
    ...
    const deleteButton = document.querySelectorAll('.delete');
      deleteButton.forEach((dBtn) => {
          dBtn.addEventListener('click', function (event) {
              ...
              Swal.fire({
                  title: 'Anda Yakin Menghapus Bank Ini ?',
                  text: "Data Bank : " + postTitle,
                  icon: 'warning',
